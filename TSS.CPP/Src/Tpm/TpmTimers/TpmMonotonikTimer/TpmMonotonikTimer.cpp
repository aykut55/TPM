...
CTpmMonotonikTimer::~CTpmMonotonikTimer()
{
    try
    {
...
CTpmMonotonikTimer::CTpmMonotonikTimer(CTpmSharedDevice* sharedDevice)
    : m_useSharedTpmDevice(sharedDevice != nullptr), m_sharedTpmDevice(sharedDevice)
{
    try
    {
        if (m_useSharedTpmDevice)
        {
            ...
            std::stringstream ss;
            ss << "[CTpmMonotonikTimer] uses shared CTpmSharedDevice\n";
            Log(ss.str());
        }
        else
        {
            ...
            std::stringstream ss;
            ss << "[CTpmMonotonikTimer] uses local  CTpmSharedDevice\n";
            Log(ss.str());
        }
...
CTpmSharedDevice* CTpmMonotonikTimer::GetTpmSharedDevice(void)
{
    return m_sharedTpmDevice;
}

bool CTpmMonotonikTimer::Release(void)
{
...
bool CTpmMonotonikTimer::Initialize(void)
{
    bool fncReturn = false;
