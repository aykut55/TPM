...
class CTpmCryptoRSA : public CTpmBaseClass
{
public:
    virtual ~CTpmCryptoRSA();
             CTpmCryptoRSA(CTpmSharedDevice* sharedDevice = nullptr);
    ...
    // ***********************************************************************************************************************
/*
SADECE asagikilerde verinin basına ekleme veya decypt sonrası
verinin sonundan cekme yapildi.
Digerlerine bakilacak....
*/
    // Encrypt/Decrypt (with Password TMP-RSA key)
    ...
    // Encrypt/Decrypt (with Password PIN-protected RSA key)
    ...
    bool GenerateKeyPair(UINT16 keyBits = 2048, UINT32 publicExponent = 0, const std::vector<BYTE>& authValue = {});
    bool CTpmCryptoRSA::GenerateKeyPairEx(
        UINT16 keyBits,
        TPM_ALG_ID hashAlg,
        UINT32 publicExponent,
        const std::vector<BYTE>& authValue);
    bool Encrypt(const std::vector<BYTE>& inputData, std::vector<BYTE>& encryptedData);
    bool Decrypt(const std::vector<BYTE>& encryptedData, std::vector<BYTE>& decryptedData);

    bool ExportPublicKeyRaw(std::vector<BYTE>& modulusOut, UINT32& exponentOut);
    bool ConvertPublicKeyToPEM(std::string& pemOut);
    // ***********************************************************************************************************************

    bool IsPasswordValidForCurrentRsaKey(const std::string& password);
    // ***********************************************************************************************************************
...
private:
    ...
    TPM_HANDLE m_keyHandle = TPM_RH_NULL;
    TPM_HANDLE m_rsaKeyHandle;

    bool m_keyGenerated = false;
    TPMT_PUBLIC m_publicArea;
    std::vector<BYTE> m_authValue;
    std::vector<BYTE> m_currentAuthValue;

    bool m_keyLoaded = false;
    bool EnsureRsaKeyLoaded();
    bool FlushKey();

    std::vector<BYTE> AddLengthPrefix(const std::vector<BYTE>& data);
    bool RemoveLengthPrefix(const std::vector<BYTE>& dataWithLength, std::vector<BYTE>& originalData);
};