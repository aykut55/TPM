    ...
    ESYS_CONTEXT *esysContext, ESYS_TR *sessionHandle)
{
    ...
    *sessionHandle = esysContext->esys_handle_cnt++;
    ...
    if (esysContext->in.StartAuthSession.bind != ESYS_TR_NONE || esysContext->salt.size > 0) {
        ...
        /* Only compute the session key if there is a tpmKey and/or a bind object.
        Otherwise the specification says the HMAC key is limited to the authValue */
        if ((tpmKey != ESYS_TR_NONE) || (bind != ESYS_TR_NONE && bindNode != NULL))
        {
            /* compute session key */
